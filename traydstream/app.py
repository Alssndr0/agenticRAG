import gradio as gr
from generation import convert_and_cache_pdf, answer_question
...
with gr.Blocks(css=css, title="PDF Agent QA") as demo:
    ...
    with gr.Row():
        file_input = gr.File(
            label="Upload PDF file", file_types=[".pdf"], type="filepath", visible=True, scale=6
        )
    ...
    with gr.Row():
        ...
        submit_btn = gr.Button("Send", elem_id="submit-button", scale=1, interactive=False)
